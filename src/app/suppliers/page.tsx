        ...
        {!selectedSupplier ? (
                  ...
                  <TableRow key={supp.id} className="cursor-pointer hover:bg-primary/5 transition-colors h-14" onClick={() => setSelectedSupplier(supp)}>
                    ...
                    <TableCell className="text-right px-4">
                      <div className="flex gap-1 justify-end">
                        ...
                        <Button variant="ghost" size="icon" className="h-9 w-9 text-rose-500" onClick={(e) => { e.stopPropagation(); deleteSupplier(supp.id); }}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
          ...
          <div className="bg-white border-2 border-black rounded-sm w-full max-w-[210mm] mx-auto p-4 sm:p-10 flex flex-col items-center animate-in slide-in-from-right-2 duration-300 relative shadow-2xl">
             ...
             <div className="w-full border-b-[4px] border-black pb-3 mb-8 text-center">
               ...
               <p className="text-[11pt] font-black text-muted-foreground uppercase tracking-widest mt-1">आयडी: {selectedSupplier.supplierId} | सविस्तर अहवाल</p>
             </div>
             ...
             <div className="w-full mt-auto pt-24 grid grid-cols-2 gap-20 text-center uppercase font-black text-[11pt] tracking-[0.2em]">
               ...
               <div className="border-t-2 border-black pt-3">सप्लायर स्वाक्षरी</div>
             </div>
          </div>
        )}
      </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-[98vw] w-[98vw] p-0 overflow-hidden rounded-2xl border-none shadow-2xl bg-white flex flex-col h-[92vh] text-left">
          <DialogHeader className="p-4 bg-primary text-white shrink-0">
            ...
            <DialogDescription className="text-[9px] text-white/70 uppercase">संपूर्ण १६+ कलमी सविस्तर फॉर्म (४-वे स्क्रोल उपलब्ध)</DialogDescription>
          </DialogHeader>
              ...
              <div className="max-w-[900px] mx-auto space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div className="sm:col-span-2 space-y-1"><Label className="text-[10px] font-black uppercase">रूट निवडा</Label>
                      <Select value={formData.routeId} onValueChange={val => setFormData({...formData, routeId: val})}>
                        <SelectTrigger className="h-10 border-2 border-black font-black"><SelectValue placeholder="रूट निवडा" /></SelectTrigger>
                        <SelectContent>
                      ...
                      {formData.sub_gavali_info.map((sg: any, idx: number) => (
                        <Card key={sg.id} className="border-2 border-indigo-100 overflow-hidden rounded-2xl shadow-md bg-white">
                          ...
                          <div className="p-3 grid grid-cols-2 gap-2">
                             <div className="space-y-1"><Label className="text-[9px] font-black uppercase">नाव</Label><Input value={sg.name} onChange={e => updateRow('sub_gavali_info', sg.id, { name: e.target.value })} className="h-8 text-[11px] border-black" /></div>
                             <div className="space-y-1"><Label className="text-[9px] font-black uppercase">मोबाईल</Label><Input value={sg.mobile} onChange={e => updateRow('sub_gavali_info', sg.id, { mobile: e.target.value })} className="h-8 text-[11px] border-black" /></div>
                          </div>
                        </Card>
                      ))}
                    </div>
